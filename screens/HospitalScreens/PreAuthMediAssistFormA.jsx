...
function noSelected(value) {
  return String(value ?? "").toLowerCase().trim() === "no";
}

function relationshipMatches(value, option) {
  const rel = String(value ?? "").toLowerCase().trim();
  if (!rel) return false;
  if (option === "other") {
...
function occupationMatches(value, option) {
  const occ = String(value ?? "").toLowerCase().trim();
  if (!occ) return false;
  if (option === "other") {
...
function roomCategoryMatches(value, option) {
  ...
  if (option === "twin sharing") return room.includes("twin") || room.includes("double");
...
function hospitalizationCauseMatches(value, option) {
  ...
  if (option === "illness") return cause.includes("ill") || cause.includes("disease");
  if (option === "maternity") {
    return cause.includes("matern") || cause.includes("deliver") || cause.includes("pregnan");
  }
  return false;
}
            ...
            <View style={{ flex: 1 }}>
              <View style={styles.rowBetween}>
                <View style={styles.inlineRow}>
                  ...
                  <ToggleCheckbox
                    checked={yesSelected(form.bCurrentlyOther)}
                    onPress={() =>
                      setField(
                        "bCurrentlyOther",
                        yesSelected(form.bCurrentlyOther) ? "" : "yes",
                      )
                    }
                  />
                  <Text style={styles.smallText}>Yes</Text>
                  <ToggleCheckbox
                    checked={noSelected(form.bCurrentlyOther)}
                    onPress={() =>
                      setField(
                        "bCurrentlyOther",
                        noSelected(form.bCurrentlyOther) ? "" : "no",
                      )
                    }
                  />
                ...
                <View style={styles.inlineRow}>
                  ...
                  <CharBoxRow
                    length={8}
                    value={form.bCommencement}
                    onChange={(v) => setField("bCommencement", v)}
                  />
                </View>
              </View>
              ...
              <View style={styles.rowBetween}>
                <View style={styles.inlineRow}>
                  ...
                  <CharBoxRow
                    length={20}
                    value={form.bIfYesCoName}
                    onChange={(v) => setField("bIfYesCoName", v)}
                  />
                </View>
                <View style={styles.inlineRow}>
                  ...
                  <CharBoxRow
                    length={18}
                    value={form.bIfYesPolicy}
                    onChange={(v) => setField("bIfYesPolicy", v)}
                  />
                </View>
              </View>
              ...
              <View style={styles.rowBetween}>
                <View style={styles.inlineRow}>
                  ...
                  <CharBoxRow
                    length={12}
                    value={form.bSumInsured}
                    onChange={(v) => setField("bSumInsured", v)}
                  />
                </View>
                <View style={styles.inlineRow}>
                  <Text style={styles.label}>
                  ...
                  <ToggleCheckbox
                    checked={yesSelected(form.bHosp4Y)}
                    onPress={() =>
                      setField("bHosp4Y", yesSelected(form.bHosp4Y) ? "" : "yes")
                    }
                  />
                  <Text style={styles.smallText}>Yes</Text>
                  <ToggleCheckbox
                    checked={noSelected(form.bHosp4Y)}
                    onPress={() =>
                      setField("bHosp4Y", noSelected(form.bHosp4Y) ? "" : "no")
                    }
                  />
                  <Text style={styles.smallText}>No</Text>
                  ...
                  <CharBoxRow
                    length={4}
                    value={form.bHosp4YDate}
                    onChange={(v) => setField("bHosp4YDate", v)}
                  />
                </View>
              </View>
              ...
              <View style={styles.rowBetweenDiagnosis}>
                ...
                <View style={styles.inlineRow}>
                  ...
                  <ToggleCheckbox
                    checked={yesSelected(form.bPreviouslyOther)}
                    onPress={() =>
                      setField(
                        "bPreviouslyOther",
                        yesSelected(form.bPreviouslyOther) ? "" : "yes",
                      )
                    }
                  />
                  <Text style={styles.smallText}>Yes</Text>
                  <ToggleCheckbox
                    checked={noSelected(form.bPreviouslyOther)}
                    onPress={() =>
                      setField(
                        "bPreviouslyOther",
                        noSelected(form.bPreviouslyOther) ? "" : "no",
                      )
                    }
                  />
                  <Text style={styles.smallText}>No</Text>
              ...
              <View style={styles.inlineRow}>
                ...
                <CharBoxRow
                  length={22}
                  value={form.bIfYesCoName2}
                  onChange={(v) => setField("bIfYesCoName2", v)}
                />
              </View>
            </View>
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={styles.inlineRow}>
                ...
                {["self", "spouse", "child", "father", "mother", "other"].map(
                  (item) => (
                    <React.Fragment key={item}>
                      ...
                      <ToggleCheckbox
                        checked={relationshipMatches(form.relationship, item)}
                        onPress={() =>
                          setField(
                            "relationship",
                            relationshipMatches(form.relationship, item)
                              ? ""
                              : item === "other"
                                ? "OTHER"
                                : item.toUpperCase(),
                ...
                <TextInput
                  style={styles.longInputWide}
                  value={
                    relationshipMatches(form.relationship, "other")
                      ? form.relationship
                      : ""
                  }
                  onChangeText={(t) => setField("relationship", t)}
                />
              </View>
              ...
              <View style={styles.inlineRow}>
                ...
                {[
                  ...
                  <React.Fragment key={item}>
                    ...
                    <ToggleCheckbox
                      checked={occupationMatches(form.occupation, item.toLowerCase())}
                      onPress={() =>
                        setField(
                          "occupation",
                          occupationMatches(form.occupation, item.toLowerCase())
                            ? ""
                            : item.toUpperCase(),
                        )
                      }
                ...
                <TextInput
                  style={styles.longInputWide}
                  value={
                    occupationMatches(form.occupation, "other")
                      ? form.occupation
                      : ""
                  }
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={styles.inlineRow}>
                ...
                {[
                  ...
                  <React.Fragment key={item}>
                    <Text style={styles.smallText}>{item}</Text>
                    <ToggleCheckbox
                      checked={roomCategoryMatches(form.roomCategory, item.toLowerCase())}
                      onPress={() => setField("roomCategory", item)}
                    />
                  </React.Fragment>
                ))}
              </View>
              ...
              <View style={styles.rowBetween}>
                <View style={styles.inlineRow}>
                  ...
                  {["Injury", "Illness", "Maternity"].map((item) => (
                    <React.Fragment key={item}>
                      <Text style={styles.smallText}>{item}</Text>
                      <ToggleCheckbox
                        checked={hospitalizationCauseMatches(
                          form.hospitalizationCause,
                          item.toLowerCase(),
                        )}
                        onPress={() => setField("hospitalizationCause", item)}
                      />
                    </React.Fragment>
                  ))}
                </View>
              ...
              <View style={styles.rowBetween}>
                ...
                <View style={styles.inlineRow}>
                  ...
                  <ToggleCheckbox
                    checked={yesSelected(form.firAttached)}
                    onPress={() =>
                      setField(
                        "firAttached",
                        yesSelected(form.firAttached) ? "" : "yes",
                      )
                    }
                  />
                  <Text style={styles.smallText}>Yes</Text>
                  <ToggleCheckbox
                    checked={noSelected(form.firAttached)}
                    onPress={() =>
                      setField(
                        "firAttached",
                        noSelected(form.firAttached) ? "" : "no",
                      )
                    }
                  />
                  <Text style={styles.smallText}>No</Text>
                ...
                <View style={styles.inlineRow}>
                  ...
                  <TextInput
                    style={styles.longInputWide}
                    value={form.systemOfMedicine}
                    onChangeText={(t) => setField("systemOfMedicine", t)}
                  />
                </View>
              </View>
            </View>
          ...
          <View style={styles.sectionContentRow}>
            <View style={{ flex: 1 }}>
              ...
              <View style={styles.rowBetween}>
                ...
                <CharBoxRow
                  length={8}
                  value={form.healthCheckupCost}
                  onChange={(v) => setField("healthCheckupCost", v)}
                />
              </View>
              ...
              <View style={styles.rowBetween}>
                ...
                <CharBoxRow
                  length={8}
                  value={form.ambulanceCharges}
                  onChange={(v) => setField("ambulanceCharges", v)}
                />
                <Text style={styles.label}>vi. Others (code):</Text>
                <CharBoxRow
                  length={3}
                  value={form.otherChargesCode}
                  onChange={(v) => setField("otherChargesCode", v)}
                />
                <Text style={styles.label}>Rs.</Text>
                <CharBoxRow
                  length={8}
                  value={form.otherChargesAmount}
                  onChange={(v) => setField("otherChargesAmount", v)}
                />
              </View>
              ...
              <View style={styles.inlineRows}>
                ...
                <CharBoxRow
                  ...
                  onChange={(v) => setField("totalClaim", v)}
                  boxStyle={styles.squareBoxs}
                />
              </View>
              ...
              <View style={styles.rowBetween}>
                <View style={styles.inlineRow}>
                  ...
                  <CharBoxRow
                    length={3}
                    value={form.preHospPeriodDays}
                    onChange={(v) => setField("preHospPeriodDays", v)}
                  />
                </View>
                <View style={styles.inlineRow}>
                  <Text style={styles.label}>
                  ...
                  <CharBoxRow
                    length={3}
                    value={form.postHospPeriodDays}
                    onChange={(v) => setField("postHospPeriodDays", v)}
                  />
                </View>
              </View>
              ...
              <View style={styles.inlineRow}>
                ...
                <ToggleCheckbox
                  checked={yesSelected(form.domiciliary)}
                  onPress={() =>
                    setField("domiciliary", yesSelected(form.domiciliary) ? "" : "yes")
                  }
                />
                <Text style={styles.smallText}>Yes</Text>
                <ToggleCheckbox
                  checked={noSelected(form.domiciliary)}
                  onPress={() =>
                    setField("domiciliary", noSelected(form.domiciliary) ? "" : "no")
                  }
                />
                <Text style={styles.smallText}>No</Text>
                <Text style={styles.smallText}>
              ...
              <View style={styles.rowBetween}>
                ...
                <CharBoxRow
                  length={8}
                  value={form.hospitalDailyCash}
                  onChange={(v) => setField("hospitalDailyCash", v)}
                />
                <Text style={styles.label}>ii. Surgical Cash Rs.</Text>
                <CharBoxRow
                  length={8}
                  value={form.surgicalCash}
                  onChange={(v) => setField("surgicalCash", v)}
                />
              </View>
              ...
              <View style={styles.rowBetween}>
                ...
                <CharBoxRow
                  length={8}
                  value={form.criticalIllnessBenefit}
                  onChange={(v) => setField("criticalIllnessBenefit", v)}
                />
                <Text style={styles.label}>iv. Convalescence Rs.</Text>
                <CharBoxRow
                  length={8}
                  value={form.convalescence}
                  onChange={(v) => setField("convalescence", v)}
                />
              </View>
              ...
              <View style={styles.rowBetween}>
                ...
                <CharBoxRow
                  length={8}
                  value={form.prePostLumpSum}
                  onChange={(v) => setField("prePostLumpSum", v)}
                />
                <Text style={styles.label}>vi. Others Rs.</Text>
                <CharBoxRow
                  length={8}
                  value={form.othersLump}
                  onChange={(v) => setField("othersLump", v)}
                />
              </View>
            </View>
            ...
            <View style={styles.sectionEChecklist}>
              ...
              {[
                ...
                <View key={i} style={styles.inlineRow}>
                  <ToggleCheckbox
                    checked={!!form.docChecklist?.[i]}
                    onPress={() => {
                      const next = [...(form.docChecklist ?? [])];
                      next[i] = !next[i];
            ...
            <View style={styles.tableContainer}>
              ...
              {Array.from({ length: 10 }).map((_, rowIndex) => (
                <View key={rowIndex} style={styles.tableRow}>
                  ...
                  <View style={[styles.tableCellInputWrap, { width: 80 }]}>
                    <TextInput
                      style={styles.tableInput}
                      value={form.billsRows?.[rowIndex]?.billNo ?? ""}
                      onChangeText={(t) => {
                        const next = [...(form.billsRows ?? [])];
                        next[rowIndex] = { ...(next[rowIndex] ?? {}), billNo: t };
                        setField("billsRows", next);
                      }}
                    />
                  </View>
                  <View style={[styles.tableCell, { width: 120, justifyContent: "center" }]}>
                    <CharBoxRow
                      length={8}
                      value={form.billsRows?.[rowIndex]?.date ?? ""}
                      onChange={(v) => {
                        const next = [...(form.billsRows ?? [])];
                        next[rowIndex] = { ...(next[rowIndex] ?? {}), date: v };
                        setField("billsRows", next);
                      }}
                    />
                  </View>
                  <View style={[styles.tableCellInputWrap, { width: 200 }]}>
                    <TextInput
                      style={styles.tableInput}
                      value={form.billsRows?.[rowIndex]?.issuedBy ?? ""}
                      onChangeText={(t) => {
                        const next = [...(form.billsRows ?? [])];
                        next[rowIndex] = { ...(next[rowIndex] ?? {}), issuedBy: t };
                        setField("billsRows", next);
                      }}
                    />
                  </View>
                  <View style={[styles.tableCellInputWrap, { flex: 1 }]}>
                    <TextInput
                      style={styles.tableInput}
                      value={
                        form.billsRows?.[rowIndex]?.towards ??
                        ""
                      }
                      onChangeText={(t) => {
                        const next = [...(form.billsRows ?? [])];
                        next[rowIndex] = { ...(next[rowIndex] ?? {}), towards: t };
                        setField("billsRows", next);
                      }}
                      placeholder={
                        rowIndex === 0
                  ...
                  <View style={[styles.tableCell, { width: 200, justifyContent: "center" }]}>
                    <CharBoxRow
                      length={10}
                      value={form.billsRows?.[rowIndex]?.amount ?? ""}
                      onChange={(v) => {
                        const next = [...(form.billsRows ?? [])];
                        next[rowIndex] = { ...(next[rowIndex] ?? {}), amount: v };
                        setField("billsRows", next);
                      }}
                    />
                  </View>
