...
const AddPatientForm = ({
  ...
  const buildSavedPatient = (response) => {
    ...
    return {
      ...
      rawPatient: {
        ...
        doctor_id: selectedDoctorObj?.id || responsePatient.doctor_id || null,
      },
    };
  };

  const assignDoctorIfSelected = async (userId) => {
    const selectedDoctorObj = doctors.find((d) => d.name === form.doctor);
    if (!selectedDoctorObj?.id) return;
    ...
    try {
      ...
      fd.append("doctor_id", selectedDoctorObj.id);

      await fetch(`${API_URL}/hospitals/staff/update_patient`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: fd,
      });
    } catch (err) {
      console.error("assignDoctorIfSelected error:", err);
    }
  };
  ...
  const persistPatient = async (requireHospitalBill = true) => {
    ...
    try {
      ...
      await assignDoctorIfSelected(savedPatient.user_id);
...
const PatientDetailPanel = ({ patient, onClose, navigation, doctors = [] }) => {
