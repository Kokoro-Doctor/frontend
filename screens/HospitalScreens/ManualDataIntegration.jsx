...
const ManualDataIntegration = ({ navigation }) => {
  ...
  // ─────────────────────────────────────────────
  // REMOVE A SELECTED DOCUMENT (cancel cross)
  // ─────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────
  // SAVE PATIENT (+ upload docs after)
  // ─────────────────────────────────────────────
  const savePatient = async () => {
    if (!patientForm.fullName?.trim()) {
      alert("Patient name is required");
  ...
  // ─────────────────────────────────────────────
  // DOCUMENT UPLOAD ROW — shared between web & mobile
  // ─────────────────────────────────────────────
  const renderDocumentUploadRow = (isMobile = false) => (
    <View style={isMobile ? styles.mobileDocRow : styles.docRow}>
      {DOC_CONFIG.map((cfg) => {
        const docState = patientDocs[cfg.key];
        const isUploaded = docState?.uploaded;
        const isUploading = docState?.uploading || docsUploading;
        ...
        return (
          <TouchableOpacity
            key={cfg.key}
            style={[
              isMobile ? styles.mobileDocBox : styles.docBox,
              isUploaded && styles.docBoxUploaded,
              hasError && styles.docBoxError,
            ]}
            onPress={() => {
          ...
          >
            {/* ── Cancel / remove button — top-right corner, only when file selected ── */}
            {hasFile && !isUploading && (
              <TouchableOpacity
                style={styles.docCancelBtn}
                onPress={(e) => {
                  // Stop event bubbling so the box onPress doesn't re-open picker
                  e?.stopPropagation?.();
                  removeDoc(cfg.key);
                }}
                ...
                activeOpacity={0.7}
              >
                <Text style={styles.docCancelBtnText}>✕</Text>
              </TouchableOpacity>
            )}

            {/* Uploaded ✓ badge — only after backend confirms, no file selected */}
            {isUploaded && !hasFile && (
              <View style={styles.docStatusBadge}>
                <Text style={styles.docStatusBadgeText}>✓</Text>
              </View>
            )}

            {/* Error ! badge */}
            {hasError && (
              <View style={[styles.docStatusBadge, styles.docStatusBadgeErr]}>
                <Text style={styles.docStatusBadgeText}>!</Text>
              </View>
            )}
            ...
            {/* File name or prompt or spinner */}
            {isUploading ? (
              <ActivityIndicator
                color="#2563EB"
                size="small"
              ...
              <Text style={styles.docPrompt}>
                Tap to upload{"\n"}
                <Text style={styles.docFormats}>PDF / JPG / PNG</Text>
              </Text>
            )}

            {/* Error message */}
            {hasError && (
              <Text style={styles.docErrorText} numberOfLines={2}>
                {docState.error}
              </Text>
            )}
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  docStatusBadgeErr: { backgroundColor: "#EF4444" },
  docStatusBadgeText: { color: "#fff", fontSize: 10, fontWeight: "700" },
  docCancelBtn: {
    position: "absolute",
    top: 6,
    right: 6,
  ...
  docIcon: { fontSize: 22, marginBottom: 6 },
  docLabel: {
    fontSize: 15,
    fontWeight: "700",
    color: "#1D4ED8",
  ...
  docFormats: { fontSize: 11, color: "#9CA3AF" },
  docErrorText: {
    fontSize: 9,
    color: "#DC2626",
    textAlign: "center",
