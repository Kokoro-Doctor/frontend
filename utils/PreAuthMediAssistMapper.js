...
export function splitAddress(raw, w1, w2) {
  ...
  return [padChars(t.slice(0, w1), w1), padChars(t.slice(w1, w1 + w2), w2)];
}
...
/** Return the first non-empty candidate, preserving booleans/numbers. */
...
/** Normalize truthy/falsy text into boolean or null. */
...
function mapChecklistFlags(items, len) {
  const flags = Array(len).fill(false);
  if (!Array.isArray(items)) return flags;
  items.forEach((item, idx) => {
    if (idx >= len) return;
    const parsed = parseBool(item);
    flags[idx] = parsed === true;
  });
  return flags;
}

function normalizeBillRows(rows, count = 10) {
  ...
  rows.slice(0, count).forEach((row, idx) => {
    if (!row || typeof row !== "object") return;
    base[idx] = {
      billNo: String(
        firstFilled(row.bill_number, row.billNo, row.invoice_no, row.invoice) ??
          "",
      ),
      date: String(
        firstFilled(row.bill_date, row.date, row.invoice_date) ?? "",
      ),
      issuedBy: String(
        firstFilled(row.issued_by, row.provider, row.hospital, row.vendor) ??
          "",
      ),
      towards: String(
        firstFilled(row.towards, row.description, row.type, row.category) ?? "",
      ),
      amount: String(
        firstFilled(row.amount, row.bill_amount, row.total_amount, row.total) ??
          "",
      ),
    };
  });
...
function extractShared(data) {
  ...
  return {
    ...
    ageMonths: padChars(
      digitsOnly(String(patient.age_months ?? "")).slice(0, 2) || "  ",
      2,
    ),
    ...
    occupation: String(patient.occupation ?? "")
      .replace(/\s+/g, " ")
      .trim(),
    relationship: String(patient.relationship_to_insured ?? "")
      .replace(/\s+/g, " ")
      .trim(),
    ...
    roomCategory: String(hosp.room_category ?? hosp.room_type ?? "")
      .replace(/\s+/g, " ")
      .trim(),
    typeOfAdmission: mapAdmissionType(
      hosp.admission_type ?? hosp.type_of_admission,
    ),
    dischargeStatus: mapDischargeStatus(hosp.status_at_discharge),

    // ── Diagnosis ─────────────────────────────────────────────
    primaryDiagnosis: String(diag.primary_diagnosis ?? "").trim(),
    hospitalizationCause: String(diag.hospitalization_cause ?? "")
      .replace(/\s+/g, " ")
      .trim(),
    systemOfMedicine: String(diag.system_of_medicine ?? "")
      .replace(/\s+/g, " ")
      .trim(),
    injurySelf: !!diag.is_self_inflicted,
    injuryRTA: !!(
      diag.is_rta ||
      String(diag.injury_cause ?? "")
        .toLowerCase()
        .includes("road")
    ),
    injurySubstance: !!(
    ...
    injuryHospitalization: yesNo(
      diag.hospitalization_due_to_injury ??
        (diag.hospitalization_cause?.toLowerCase().includes("injur")
          ? true
          : null),
    ),
    firstConsultationDate: padChars(
      parseDateDDMMYYYY(diag.first_consultation_date),
      8,
    ),
    expectedDeliveryDate: padChars(
      parseDateDDMMYYYY(diag.expected_delivery_date),
      8,
    ),
    dateOfDelivery: padChars(parseDateDDMMYY(diag.date_of_delivery), 6),
    primaryIcdCode: padChars(
      truncate(upper(diag.primary_icd_code).replace(/\s/g, ""), 10),
      10,
    ),
    primaryIcdPcsCode: padChars(
      truncate(
        upper(
    ...
    treatingDoctorContact: padChars(
      truncate(
        digitsOnly(
          firstFilled(
            hosp.doctor_phone,
    ...
    hospitalLocation: upper(
      joinLocation(
        firstFilled(hosp.hospital_city, hosp.city),
        firstFilled(hosp.hospital_state, hosp.state),
      ),
      26,
    ),
  };
...
function mapDischargeStatus(raw) {
  ...
  if (s.includes("another") || s.includes("transfer"))
...
function applyAutofillOverlayA(base, ar) {
  ...
  if (a.policy_number)
    out.policyNumber = padChars(
      truncate(upper(a.policy_number).replace(/\s/g, ""), 18),
      18,
    );
  if (a.certificate_number)
    out.certificateNumber = padChars(
      truncate(upper(a.certificate_number).replace(/\s/g, ""), 14),
      14,
    );
  if (a.company_tpa_id) out.tpaId = padChars(upper(a.company_tpa_id, 22), 22);
  if (a.name) {
    out.primaryName = padChars(upper(a.name, 40), 40);
    out.hospitalizedName = out.primaryName;
  }
  if (a.address) {
    const [l1, l2] = splitAddress(a.address, 40, 35);
  ...
  if (a.pin_code)
    out.primaryPin = padChars(truncate(digitsOnly(a.pin_code), 6), 6);
  if (a.phone)
    out.primaryPhone = padChars(truncate(digitsOnly(a.phone), 10), 10);
  if (a.email) out.primaryEmail = String(a.email).trim();

  // Section B
  if (b.currently_other_insurance != null)
    out.bCurrentlyOther = yesNo(b.currently_other_insurance);
  if (b.commencement_date)
    out.bCommencement = padChars(parseDateDDMMYYYY(b.commencement_date), 8);
  if (b.company_name)
    out.bIfYesCoName = padChars(upper(b.company_name, 20), 20);
  if (b.policy_number)
    out.bIfYesPolicy = padChars(
      truncate(upper(b.policy_number).replace(/\s/g, ""), 18),
      18,
    );
  if (b.sum_insured) out.bSumInsured = padAmount(b.sum_insured, 12);
  if (b.hospitalized_last_4_years != null)
    out.bHosp4Y = yesNo(b.hospitalized_last_4_years);
  if (b.diagnosis) out.diagnosis = String(b.diagnosis);
  if (b.previously_other_insurance != null)
    out.bPreviouslyOther = yesNo(b.previously_other_insurance);

  // Section C
  if (c.name) {
    out.hospitalizedName = padChars(upper(c.name, 40), 40);
  }
  if (c.gender) {
    const g = c.gender.toLowerCase();
    out.gender = g.includes("f")
      ? "female"
      : g.includes("m")
        ? "male"
        : out.gender;
  ...
  if (c.age_years != null)
    out.ageYears = padChars(parseAge(String(c.age_years)), 2);
  if (c.age_months != null)
    out.ageMonths = padChars(truncate(digitsOnly(String(c.age_months)), 2), 2);
  if (c.date_of_birth)
    out.dob = padChars(parseDateDDMMYYYY(c.date_of_birth), 8);
  ...
  if (c.address) {
    const [c1, c2] = splitAddress(c.address, 40, 35);
  ...
  // Section D
  if (d.hospital_name)
    out.hospitalName = padChars(upper(d.hospital_name, 40), 40);
  ...
  if (d.date_of_injury_or_disease)
    out.injuryDate = padChars(
      parseDateDDMMYYYY(d.date_of_injury_or_disease),
      8,
    );
  if (d.admission_date)
    out.admissionDate = padChars(parseDateDDMMYYYY(d.admission_date), 8);
  if (d.admission_time) out.admissionTime = parseTime(d.admission_time);
  if (d.discharge_date)
    out.dischargeDate = padChars(parseDateDDMMYYYY(d.discharge_date), 8);
  ...
  if (d.treating_doctor_contact)
    out.treatingDoctorContact = padChars(
      truncate(digitsOnly(d.treating_doctor_contact), 10),
      10,
    );
  if (d.first_consultation_date)
    out.firstConsultationDate = padChars(
      parseDateDDMMYYYY(d.first_consultation_date),
      8,
    );
  if (d.primary_icd_code)
    out.icd10Code = padChars(
      truncate(upper(d.primary_icd_code).replace(/\s/g, ""), 10),
      10,
    );
  if (d.primary_icd_pcs_code)
    out.icd10PcsCode = padChars(
      truncate(upper(d.primary_icd_pcs_code).replace(/\s/g, ""), 10),
      10,
    );
  if (d.date_of_delivery_expected)
    out.expectedDeliveryDate = padChars(
      parseDateDDMMYYYY(d.date_of_delivery_expected),
      8,
    );
  if (d.is_rta === true) out.injuryRta = true;
  if (d.reported_to_police === true) out.reportedPolice = true;
  if (d.reported_to_police === false) out.reportedPolice = false;
  if (d.fir_attached != null) out.firAttached = yesNo(d.fir_attached);
  if (d.injury_cause) {
  ...
  if (e.pre_hospitalization_expenses != null)
    out.claimPre = padAmount(e.pre_hospitalization_expenses, 8);
  if (e.hospitalization_expenses != null)
    out.claimHospital = padAmount(e.hospitalization_expenses, 8);
  if (e.post_hospitalization_expenses != null)
    out.claimPost = padAmount(e.post_hospitalization_expenses, 8);
  if (e.health_checkup_cost != null)
    out.healthCheckupCost = padAmount(e.health_checkup_cost, 8);
  if (e.ambulance_charges != null)
    out.ambulanceCharges = padAmount(e.ambulance_charges, 8);
  if (e.other_charges != null)
    out.otherChargesAmount = padAmount(e.other_charges, 8);
  ...
  if (e.pre_hospitalization_period_days != null)
    out.preHospPeriodDays = padChars(
      truncate(String(e.pre_hospitalization_period_days), 3),
      3,
    );
  if (e.post_hospitalization_period_days != null)
    out.postHospPeriodDays = padChars(
      truncate(String(e.post_hospitalization_period_days), 3),
      3,
    );
  if (e.domiciliary_hospitalization != null)
    out.domiciliary = yesNo(e.domiciliary_hospitalization);
  if (e.hospital_daily_cash)
    out.hospitalDailyCash = padAmount(e.hospital_daily_cash, 8);
  if (e.surgical_cash) out.surgicalCash = padAmount(e.surgical_cash, 8);
  if (e.critical_illness_benefit)
    out.criticalIllnessBenefit = padAmount(e.critical_illness_benefit, 8);
  if (e.convalescence) out.convalescence = padAmount(e.convalescence, 8);
  if (e.pre_post_hospitalization_lump_sum != null)
    out.prePostLumpSum = padAmount(e.pre_post_hospitalization_lump_sum, 8);
  if (e.others_lump != null) out.othersLump = padAmount(e.others_lump, 8);
  if (Array.isArray(e.documents_checklist)) {
    out.docChecklist = mapChecklistFlags(e.documents_checklist, 13);
  }

  // Section G (bank)
  if (g.pan)
    out.pan = padChars(truncate(upper(g.pan).replace(/\s/g, ""), 10), 10);
  if (g.account_number)
    out.accountNumber = padChars(
      truncate(String(g.account_number).replace(/\s/g, ""), 22),
      22,
    );
  if (g.bank_name_branch)
    out.bankNameBranch = padChars(upper(String(g.bank_name_branch), 40), 40);
  if (g.cheque_dd_payable_to)
    out.chequeDetails = String(g.cheque_dd_payable_to).trim();
  if (g.ifsc_code)
    out.ifscCode = padChars(truncate(upper(g.ifsc_code), 11), 11);

  // Part B overrides
  if (partB.patient_name)
    out.hospitalizedName = padChars(upper(partB.patient_name, 40), 40);
  if (partB.primary_diagnosis) out.diagnosis = String(partB.primary_diagnosis);
  if (partB.total_claimed_amount)
    out.claimHospital = padAmount(partB.total_claimed_amount, 8);
  ...
  if (Array.isArray(sF)) {
    sF.forEach((row, i) => {
      if (i >= 10 || !row) return;
      out.billsRows[i] = normalizeBillRows([row], 1)[0];
    });
  }
...
function enrichFormAFields(form, data, analysisData) {
  const patient = data.patient_details ?? {};
  const hosp = data.hospital_details ?? {};
  ...
  const treatingDoctorName = String(
    firstFilled(
      form.treatingDoctor,
      hosp.treating_doctor,
      partB.treating_doctor,
    ) ?? "",
  ).trim();
  const treatingDoctorContact = padChars(
    truncate(
      digitsOnly(
        firstFilled(
          form.treatingDoctorContact,
          hosp.doctor_phone,
          hosp.treating_doctor_phone,
          hosp.hospital_phone,
          partB.treating_doctor_contact,
        ),
      ),
      10,
    ),
  ...
  const procedureExists = [
    ...
    diag.procedure_1_icd_pcs,
    diag.primary_icd_pcs_code,
  ].some((value) => String(value ?? "").trim() !== "");
  const systemOfMedicine = String(form.systemOfMedicine ?? "").toLowerCase();
  const typeOfAdmission = String(
    firstFilled(
      form.typeOfAdmission,
      hosp.admission_type,
      hosp.type_of_admission,
    ) ?? "",
  ).toLowerCase();
  const firNumber = padChars(
    truncate(
      String(
        firstFilled(form.firNumber, diag.fir_number, partB.fir_number) ?? "",
      ).replace(/\s/g, ""),
      8,
    ),
    8,
  );
  const dateOfDeliveryExpected = padChars(
    parseDateDDMMYYYY(
      firstFilled(
        form.expectedDeliveryDate,
        diag.expected_delivery_date,
        diag.expected_date_of_delivery,
        ext.maternity_details?.expected_delivery_date,
      ),
    ),
    8,
  );
  const billsRows = form.billsRows?.some((row) =>
    Object.values(row ?? {}).some(Boolean),
  )
    ? form.billsRows
    : normalizeBillRows(
        firstFilled(
          ar.section_f_bills_enclosed,
          claim.bills,
          claim.bill_rows,
          ext.billing_details?.bills,
          ext.billing_details?.itemized_bills,
        ) ?? [],
      );
  ...
  return {
    ...
    typeOfAdmission:
      form.typeOfAdmission ??
      mapAdmissionType(hosp.admission_type ?? hosp.type_of_admission),
    firAttached: form.firAttached ?? "",
    patientName: String(
      firstFilled(
        form.patientName,
        patient.name,
        form.hospitalizedName,
        form.primaryName,
      ) ?? "",
    ).trim(),
    contactNumber: padChars(
      truncate(
        digitsOnly(
          firstFilled(
            form.contactNumber,
    ...
    altContactNumber: padChars(
      truncate(
        digitsOnly(
          firstFilled(
            form.altContactNumber,
    ...
    otherInsurance: String(
      firstFilled(form.otherInsurance, form.bCurrentlyOther) ?? "",
    ).trim(),
    insurerName: padChars(
      upper(
        String(
          firstFilled(
            form.insurerName,
    ...
    familyPhysicianName: String(
      firstFilled(
        form.familyPhysicianName,
        patient.family_physician_name,
        diag.family_physician_name,
      ) ?? "",
    ).trim(),
    familyPhysicianContact: padChars(
      truncate(
        digitsOnly(
          firstFilled(
            form.familyPhysicianContact,
            patient.family_physician_contact,
            diag.family_physician_contact,
          ),
        ),
        10,
      ),
    ...
    rohiniId: padChars(
      truncate(String(firstFilled(form.rohiniId, hosp.rohini_id) ?? ""), 18),
      18,
    ),
    hospitalLocation: padChars(
      upper(
        String(
          firstFilled(
            form.hospitalLocation,
            joinLocation(hosp.hospital_city, hosp.hospital_state),
            joinLocation(hosp.city, hosp.state),
          ) ?? "",
        ),
        26,
      ),
    ...
    employeeId: String(
      firstFilled(form.employeeId, patient.employee_id) ?? "",
    ).trim(),
    treatingDoctorName,
    treatingDoctorContact,
    firstConsultationDate: padChars(
      firstFilled(
        form.firstConsultationDate,
        padChars(parseDateDDMMYYYY(diag.first_consultation_date), 8),
      ) ?? "",
      8,
    ),
    icd10Code: padChars(
    ...
    dateOfInjury: padChars(
      firstFilled(form.dateOfInjury, form.injuryDate) ?? "",
      8,
    ),
    dateOfAdmission: padChars(
      firstFilled(form.dateOfAdmission, form.admissionDate) ?? "",
      8,
    ),
    timeOfAdmission: padChars(
      firstFilled(form.timeOfAdmission, form.admissionTime) ?? "",
      4,
    ),
    expectedDeliveryDate: dateOfDeliveryExpected,
    firNo: firNumber,
    billsRows,
    proposedTreatmentMedical:
      parseBool(
        firstFilled(
          form.proposedTreatmentMedical,
          diag.proposed_line_medical_management,
        ),
      ) === true,
    proposedTreatmentSurgical:
      parseBool(
        firstFilled(
          form.proposedTreatmentSurgical,
          diag.proposed_line_surgical_management,
        ),
      ) ?? procedureExists,
    proposedTreatmentIntensiveCare:
      parseBool(
        firstFilled(
          form.proposedTreatmentIntensiveCare,
          diag.proposed_line_intensive_care,
        ),
      ) ??
      Number(firstFilled(hosp.icu_days, ext.admission_details?.icu_days) ?? 0) >
        0,
    proposedTreatmentInvestigation:
      parseBool(
        firstFilled(
          form.proposedTreatmentInvestigation,
          diag.proposed_line_investigation,
        ),
      ) === true,
    proposedTreatmentNonAllopathic:
      parseBool(
        firstFilled(
          form.proposedTreatmentNonAllopathic,
          diag.proposed_line_non_allopathic,
        ),
      ) ?? (systemOfMedicine ? !systemOfMedicine.includes("allopath") : false),
    drugAdministrationRoute: String(
      firstFilled(
        form.drugAdministrationRoute,
        diag.drug_administration_route,
      ) ?? "",
    )
      .trim()
      .toLowerCase(),
    substanceTestDone: String(
      firstFilled(
        form.substanceTestDone,
        yesNo(diag.substance_test_conducted),
      ) ?? "",
    ).trim(),
    isEmergencyHospitalization:
      parseBool(
        firstFilled(
          ...
          diag.is_emergency_hospitalization,
        ),
    ...
    isPlannedHospitalization:
      parseBool(
        firstFilled(
          ...
          diag.is_planned_hospitalization,
        ),
      ) ?? typeOfAdmission === "planned",
  };
}
...
//     substanceTestDone:   sh.substanceTestDone,
...
//   const overlaid = analysisData?.autofill_result
//     ? applyAutofillOverlayA(base, analysisData.autofill_result)
//     : base;

//   return enrichFormAFields(overlaid, data, analysisData);
// }
// FIND and REPLACE the entire mapToFormA function:
...
export function mapToFormA(analysisData) {
  ...
  const base = {
    // ── Header ────────────────────────────────────────
    hospitalName: padChars(upper(hosp.hospital_name, 40), 40),
    hospitalLocation: sh.hospitalLocation,
    hospitalId: padChars(truncate(String(hosp.hospital_id ?? ""), 12), 12),
    hospitalEmail: sh.hospitalEmail,
    rohiniId: padChars(truncate(String(hosp.rohini_id ?? ""), 12), 12),

    // ── Section A: Insured/Patient fills ──────────────
    patientName: padChars(upper(patient.name, 38), 38),
    gender: sh.gender,
    contactNumber: sh.phone,
    altContactNumber: padChars(
      truncate(digitsOnly(patient.phone_secondary ?? ""), 10),
      10,
    ),
    ageYears: sh.ageYears,
    ageMonths: sh.ageMonths,
    ...
    otherInsurance: "",
    insurerName: padChars(upper(tpaId(ins), 18), 18),
    otherInsuranceDetails: "",
    familyPhysicianName: String(
      patient.family_physician_name ?? diag.family_physician_name ?? "",
    ).trim(),
    familyPhysicianContact: padChars(
      truncate(digitsOnly(patient.family_physician_contact ?? ""), 10),
      10,
    ),
    occupation: sh.occupation,
    ...
    // ── Section B: Treating Doctor/Hospital fills ─────
    treatingDoctorName: sh.treatingDoctor,
    treatingDoctorContact: sh.treatingDoctorContact,
    illnessComplaints: String(diag.primary_diagnosis ?? "").trim(),
    clinicalFindings: String(diag.clinical_findings ?? "").trim(),
    durationOfAilment: String(diag.duration_of_ailment ?? "").trim(),
    firstConsultationDate: sh.firstConsultationDate,
    pastHistory: String(diag.past_history ?? "").trim(),
    provisionalDiagnosis: String(diag.primary_diagnosis ?? "").trim(),
    icd10Code: sh.primaryIcdCode,
    proposedTreatmentMedical: false,
    proposedTreatmentSurgical: false,
    proposedTreatmentIntensiveCare: false,
    proposedTreatmentInvestigation: false,
    proposedTreatmentNonAllopathic: false,
    investigationDetails: "",
    drugAdministrationRoute: String(diag.drug_administration_route ?? "")
      .trim()
      .toLowerCase(),
    surgeryName: String(
      diag.procedure_1 ?? diag.procedure_details ?? "",
    ).trim(),
    icd10PcsCode: sh.primaryIcdPcsCode,
    otherTreatmentDetails: "",
    injuryOccurrence: "",
    isRta: sh.injuryRTA,
    dateOfInjury: padChars(
      parseDateDDMMYYYY(diag.date_of_injury_or_disease ?? ""),
      8,
    ),
    reportedPolice: sh.reportedPolice,
    firNo: sh.firNumber,
    injurySubstance: sh.injurySubstance,
    substanceTestDone: sh.substanceTestDone,
    maternityG: String(diag.gravida_status ?? "").trim(),
    maternityP: "",
    maternityL: "",
    maternityA: "",
    expectedDeliveryDate: sh.expectedDeliveryDate,

    // ── Section C: Patient Admitted details ───────────
    dateOfAdmission: sh.admissionDate,
    timeOfAdmission: sh.admissionTime,
    isEmergencyHospitalization: sh.typeOfAdmission === "emergency",
    isPlannedHospitalization: sh.typeOfAdmission === "planned",
    expectedDaysStay: String(hosp.expected_days_stay ?? "").trim(),
    icuDays: String(hosp.icu_days ?? "").trim(),
    roomType: sh.roomCategory,
    ...
    costIcu: "",
    costOt: "",
    costProfessionalFees: "",
    costMedicines: "",
    costOtherExpenses: "",
    costPackageCharges: "",
    costTotal: String(diag.estimated_cost ?? hosp.estimated_cost ?? "").trim(),

    // ── Page 2: Chronic illness history ──────────────
    chronicDiabetes: false,
    chronicHeartDisease: false,
    chronicHypertension: false,
    chronicHyperlipidemia: false,
    chronicOsteoarthritis: false,
    chronicAsthma: false,
    chronicCancer: false,
    chronicAlcohol: false,
    chronicHiv: false,
    // ── Page 2: Declaration fields ────────────────────
    declarationDoctorName: sh.treatingDoctor,
    qualification:         String(hosp.doctor_qualification ?? "").trim(),
    registrationNo:        String(hosp.doctor_registration_number ?? "").trim(),
    patientEmail:          sh.email,
    chronicDiabetesDate:        "",
    chronicHeartDiseaseDate:    "",
    chronicHypertensionDate:    "",
    chronicHyperlipidemiaDate:  "",
    chronicOsteoarthritisDate:  "",
    chronicAsthmaDate:          "",
    chronicCancerDate:          "",
    chronicAlcoholDate:         "",
    chronicHivDate:             "",
...
function applyAutofillOverlayB(base, analysisData, data) {
  const ar = analysisData?.autofill_result;
  if (!ar || typeof ar !== "object") return base;
  ...
  const doctorName = splitName(
    firstFilled(
      partB.treating_doctor,
      partB.treating_doctor_name,
      hosp.treating_doctor,
    ),
  );
  const patientName = splitName(firstFilled(partB.patient_name, patient.name));

  out.hospitalName = padChars(
    upper(firstFilled(partB.hospital_name, out.hospitalName), 50),
    50,
  );
  out.hospitalId = padChars(
    truncate(
      String(
        firstFilled(partB.hospital_id, hosp.hospital_id, out.hospitalId) ?? "",
      ),
      20,
    ),
    20,
  );
  const hospitalType = String(
    firstFilled(partB.hospital_type, hosp.hospital_type, out.hospitalNetwork) ??
      "",
  ).toLowerCase();
  if (hospitalType) {
    out.hospitalNetwork = hospitalType.includes("non")
      ? "non_network"
      : "network";
  }
  out.treatingDoctorSurname = doctorName.surname;
  out.treatingDoctorFirst = doctorName.first;
  out.treatingDoctorMiddle = doctorName.middle;
  out.qualification = String(
    firstFilled(
      partB.doctor_qualification,
      hosp.doctor_qualification,
      out.qualification,
    ) ?? "",
  ).trim();
  out.registrationNoStateCode = padChars(
    truncate(
      String(
        firstFilled(
          partB.doctor_registration_number,
          partB.registration_no,
          hosp.doctor_registration_number,
          out.registrationNoStateCode,
        ) ?? "",
      ),
      14,
    ),
  ...
  out.patientSurname = patientName.surname;
  out.patientFirst = patientName.first;
  out.patientMiddle = patientName.middle;
  out.ipRegNumber = padChars(
    truncate(
      String(
        firstFilled(
          partB.ip_registration_number,
          hosp.ip_registration_number,
          patient.patient_id,
          out.ipRegNumber,
        ) ?? "",
      ),
  ...
  if (partB.gender) {
    const g = String(partB.gender).toLowerCase();
    out.gender = g.includes("f")
      ? "female"
      : g.includes("m")
        ? "male"
  ...
  if (partB.age_years != null)
    out.ageYears = padChars(parseAge(partB.age_years), 2);
  if (partB.age_months != null)
    out.ageMonths = padChars(truncate(digitsOnly(partB.age_months), 2), 2);
  if (partB.date_of_birth)
    out.dob = padChars(parseDateDDMMYY(partB.date_of_birth), 6);
  if (partB.admission_date)
    out.admissionDate = padChars(parseDateDDMMYYYY(partB.admission_date), 8);
  if (partB.admission_time) out.admissionTime = parseTime(partB.admission_time);
  if (partB.discharge_date)
    out.dischargeDate = padChars(parseDateDDMMYYYY(partB.discharge_date), 8);
  if (partB.discharge_time) out.dischargeTime = parseTime(partB.discharge_time);
  if (partB.type_of_admission)
    out.typeOfAdmission = mapAdmissionType(partB.type_of_admission);
  if (partB.date_of_delivery)
    out.dateOfDelivery = padChars(parseDateDDMMYY(partB.date_of_delivery), 6);
  if (partB.gravida_status != null)
    out.gravidaStatus = padChars(truncate(String(partB.gravida_status), 4), 4);
  if (partB.status_at_discharge)
    out.dischargeStatus = mapDischargeStatus(partB.status_at_discharge);
  out.totalClaimedAmount = String(
    firstFilled(
      partB.total_claimed_amount,
      claim.bill_amount,
      claim.claimed_amount,
      out.totalClaimedAmount,
    ) ?? "",
  ).trim();

  if (Array.isArray(partB.diagnoses)) {
    out.diagnoses = Array.from({ length: 4 }, (_, idx) => {
      const row = partB.diagnoses[idx] ?? {};
      return {
        icd10: padChars(
          truncate(
            upper(firstFilled(row.icd10, row.icd_code, row.code) ?? "").replace(
              /\s/g,
              "",
            ),
            7,
        ...
        description: String(
          firstFilled(row.description, row.label, row.name) ?? "",
        ),
      };
    });
  }

  if (Array.isArray(partB.procedures)) {
    out.procedures = Array.from({ length: 4 }, (_, idx) => {
      const row = partB.procedures[idx] ?? {};
      return {
        icd10pcs: padChars(
          truncate(
            upper(
              firstFilled(row.icd10pcs, row.icd_pcs, row.code) ?? "",
            ).replace(/\s/g, ""),
            7,
          ),
          7,
  ...
  if (partB.pre_authorization_obtained != null)
    out.preAuthObtained = yesNo(partB.pre_authorization_obtained);
  if (partB.pre_authorization_number)
    out.preAuthNumber = padChars(
      truncate(String(partB.pre_authorization_number), 18),
      18,
    );
  if (partB.pre_authorization_missing_reason)
    out.preAuthMissingReason = String(
      partB.pre_authorization_missing_reason,
    ).trim();
  if (partB.hospitalization_due_to_injury != null)
    out.injuryHospitalization = yesNo(partB.hospitalization_due_to_injury);
  if (partB.injury_cause) {
    const cause = String(partB.injury_cause).toLowerCase();
    out.injurySelf = cause.includes("self");
    out.injuryRTA = cause.includes("road") || cause.includes("rta");
    out.injurySubstance =
      cause.includes("alcohol") || cause.includes("substance");
  }
  if (partB.substance_test_conducted != null)
    out.substanceTestDone = yesNo(partB.substance_test_conducted);
  if (partB.is_medico_legal != null)
    out.medicoLegal = yesNo(partB.is_medico_legal);
  if (partB.reported_to_police != null)
    out.reportedPolice = yesNo(partB.reported_to_police);
  if (partB.fir_number)
    out.firNumber = padChars(truncate(String(partB.fir_number), 12), 12);
  if (partB.fir_not_reported_reason)
    out.firNotReportedReason = String(partB.fir_not_reported_reason).trim();

  if (Array.isArray(partB.claim_documents_checklist)) {
    out.claimDocChecklist = mapChecklistFlags(
      partB.claim_documents_checklist,
      16,
    );
  }

  out.nonNetAddress1 = padChars(
    truncate(
      upper(
        firstFilled(
          partB.hospital_address_line_1,
  ...
  out.nonNetAddress2 = padChars(
    truncate(
      upper(firstFilled(partB.hospital_address_line_2, out.nonNetAddress2), 50),
      50,
    ),
    50,
  );
  out.nonNetCity = String(
    firstFilled(partB.hospital_city, hosp.hospital_city, out.nonNetCity) ?? "",
  ).trim();
  out.nonNetState = String(
    firstFilled(partB.hospital_state, hosp.hospital_state, out.nonNetState) ??
      "",
  ).trim();
  out.nonNetPin = padChars(
    truncate(
      digitsOnly(
        firstFilled(partB.hospital_pin, hosp.hospital_pin, out.nonNetPin),
      ),
      6,
    ),
    6,
  );
  out.nonNetPhone = padChars(
    truncate(
      digitsOnly(
        firstFilled(partB.hospital_phone, hosp.hospital_phone, out.nonNetPhone),
      ),
      10,
    ),
    10,
  );
  out.nonNetRegStateCode = padChars(
    truncate(
      String(
        firstFilled(
          partB.hospital_registration_number,
          hosp.hospital_registration_number,
          out.nonNetRegStateCode,
        ) ?? "",
      ),
      14,
    ),
  ...
  out.hospitalPan = padChars(
    truncate(
      upper(
        String(
          firstFilled(partB.hospital_pan, hosp.hospital_pan, out.hospitalPan) ??
            "",
        ),
      ).replace(/\s/g, ""),
      10,
  ...
  out.inpatientBeds = padChars(
    truncate(
      String(
        firstFilled(
          partB.inpatient_beds,
          hosp.inpatient_beds,
          out.inpatientBeds,
        ) ?? "",
      ),
  ...
  if (partB.has_ot != null) out.facilityOT = yesNo(partB.has_ot);
  if (partB.has_icu != null) out.facilityICU = yesNo(partB.has_icu);
  if (partB.other_facilities)
    out.otherFacilities = String(partB.other_facilities).trim();
  if (partB.declaration_date)
    out.declarationDate = padChars(
      parseDateDDMMYYYY(partB.declaration_date),
      8,
    );
  if (partB.declaration_place)
    out.declarationPlace = upper(String(partB.declaration_place).trim(), 40);
...
//     hospitalNetwork: String(hosp.hospital_type ?? "")
//       .toLowerCase()
//       .includes("non")
//       ? "non_network"
//       : "network",
...
//     qualification: String(hosp.doctor_qualification ?? "").trim(),
//     registrationNoStateCode: padChars(
//       truncate(String(hosp.doctor_registration_number ?? ""), 14),
//       14,
//     ),
//     phoneNo: padChars(
//       truncate(digitsOnly(hosp.doctor_phone ?? hosp.hospital_phone ?? ""), 11),
//       11,
//     ),
...
//     ipRegNumber: padChars(
//       truncate(
//         String(hosp.ip_registration_number ?? patient.patient_id ?? ""),
//         15,
//       ),
//       15,
//     ),
...
//     typeOfAdmission: sh.typeOfAdmission,
//     dateOfDelivery: padChars(parseDateDDMMYY(diag.date_of_delivery ?? ""), 6),
//     gravidaStatus: padChars(
//       truncate(
//         String(
//           diag.gravida_status ?? ext.maternity_details?.gravida_status ?? "",
//         ),
//         4,
//       ),
//       4,
...
//       data.claim_details?.bill_amount ??
//         data.claim_details?.claimed_amount ??
//         "",
//     ).trim(),
...
//     preAuthNumber: padChars(
//       truncate(String(hosp.pre_auth_number ?? ""), 18),
//       18,
//     ),
...
//     nonNetAddress1: padChars(
//       truncate(upper(hosp.hospital_address ?? ""), 45),
//       45,
//     ),
...
//     nonNetPhone: padChars(
//       truncate(digitsOnly(hosp.hospital_phone ?? ""), 10),
//       10,
//     ),
//     nonNetRegStateCode: padChars(
//       truncate(String(hosp.hospital_registration_number ?? ""), 14),
//       14,
//     ),
//     hospitalPan: padChars(
//       truncate(upper(hosp.hospital_pan ?? "").replace(/\s/g, ""), 10),
//       10,
//     ),
...
//     declarationDate: padChars(
//       parseDateDDMMYYYY(data.document_metadata?.document_date),
//       8,
//     ),
//     declarationPlace: upper(
//       String(hosp.hospital_city ?? patient.city ?? "").trim(),
//       40,
//     ),
//   };

//   return applyAutofillOverlayB(base, analysisData, data);
// }
