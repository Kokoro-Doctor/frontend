...
function normalizeBillRows(rows, count = 10) {
  ...
  rows.slice(0, count).forEach((row, idx) => {
    ...
    base[idx] = {
      ...
      date: String(firstFilled(row.bill_date, row.date, row.invoice_date) ?? ""),
      issuedBy: String(
...
function extractShared(data) {
  ...
  const diag    = data.diagnosis_and_procedures ?? {};
    ...
    return {
      ...
      expectedDeliveryDate:   padChars(parseDateDDMMYYYY(diag.expected_delivery_date), 8),
      dateOfDelivery:         padChars(parseDateDDMMYY(diag.date_of_delivery), 6),
      primaryIcdCode:         padChars(truncate(upper(diag.primary_icd_code).replace(/\s/g,""), 10), 10),
      primaryIcdPcsCode:      padChars(truncate(upper(firstFilled(diag.primary_icd_pcs_code, diag.procedure_1_icd_pcs)).replace(/\s/g,""), 10), 10),
      treatingDoctorContact:  padChars(truncate(digitsOnly(firstFilled(hosp.doctor_phone, hosp.treating_doctor_phone, hosp.hospital_phone)), 10), 10),
      hospitalEmail:          String(firstFilled(hosp.hospital_email, hosp.email) ?? "").trim(),
...
function applyAutofillOverlayA(base, ar) {
    ...
    if (d.treating_doctor_contact)    out.treatingDoctorContact = padChars(truncate(digitsOnly(d.treating_doctor_contact), 10), 10);
    if (d.first_consultation_date)    out.firstConsultationDate = padChars(parseDateDDMMYYYY(d.first_consultation_date), 8);
    if (d.primary_icd_code)           out.icd10Code            = padChars(truncate(upper(d.primary_icd_code).replace(/\s/g,""), 10), 10);
    if (d.primary_icd_pcs_code)       out.icd10PcsCode         = padChars(truncate(upper(d.primary_icd_pcs_code).replace(/\s/g,""), 10), 10);
    if (d.date_of_delivery_expected)  out.expectedDeliveryDate = padChars(parseDateDDMMYYYY(d.date_of_delivery_expected), 8);
...
function enrichFormAFields(form, data, analysisData) {
  ...
  const treatingDoctorName = String(
    firstFilled(form.treatingDoctor, hosp.treating_doctor, partB.treating_doctor) ?? "",
  ).trim();
  ...
  const typeOfAdmission = String(firstFilled(form.typeOfAdmission, hosp.admission_type, hosp.type_of_admission) ?? "").toLowerCase();
  const firNumber = padChars(
    truncate(
      String(firstFilled(form.firNumber, diag.fir_number, partB.fir_number) ?? "").replace(/\s/g, ""),
      8,
    ),
    8,
  );
  ...
  const billsRows = form.billsRows?.some((row) => Object.values(row ?? {}).some(Boolean))
    ? form.billsRows
    : normalizeBillRows(
        firstFilled(
  ...
  return {
    ...form,
    typeOfAdmission: form.typeOfAdmission ?? mapAdmissionType(hosp.admission_type ?? hosp.type_of_admission),
    firAttached: form.firAttached ?? "",
    patientName: String(firstFilled(form.patientName, patient.name, form.hospitalizedName, form.primaryName) ?? "").trim(),
    contactNumber: padChars(truncate(digitsOnly(firstFilled(form.contactNumber, patient.phone, patient.mobile, form.hospPhone, form.primaryPhone)), 10), 10),
    altContactNumber: padChars(truncate(digitsOnly(firstFilled(form.altContactNumber, patient.phone_secondary, patient.alt_phone)), 10), 10),
    insurerIdCardNo: padChars(truncate(String(firstFilled(form.insurerIdCardNo, ins.insurer_id_card, ins.certificate_number, form.certificateNumber) ?? "").replace(/\s/g, ""), 18), 18),
    policyNumberCorporate: padChars(truncate(String(firstFilled(form.policyNumberCorporate, ins.policy_number, form.policyNumber) ?? "").replace(/\s/g, ""), 24), 24),
    otherInsurance: String(firstFilled(form.otherInsurance, form.bCurrentlyOther) ?? "").trim(),
    insurerName: padChars(upper(String(firstFilled(form.insurerName, ins.insurance_company, ins.tpa_name, form.tpaId, form.bIfYesCoName) ?? ""), 18), 18),
    familyPhysicianName: String(firstFilled(form.familyPhysicianName, patient.family_physician_name, diag.family_physician_name) ?? "").trim(),
    familyPhysicianContact: padChars(truncate(digitsOnly(firstFilled(form.familyPhysicianContact, patient.family_physician_contact, diag.family_physician_contact)), 10), 10),
    hospitalId: padChars(truncate(String(firstFilled(form.hospitalId, hosp.hospital_id, hosp.rohini_id, partB.hospital_id) ?? ""), 18), 18),
    rohiniId: padChars(truncate(String(firstFilled(form.rohiniId, hosp.rohini_id) ?? ""), 18), 18),
    ...
    hospitalEmail: String(firstFilled(form.hospitalEmail, hosp.hospital_email, hosp.email, form.hospEmail) ?? "").trim(),
    employeeId: String(firstFilled(form.employeeId, patient.employee_id) ?? "").trim(),
    treatingDoctorName,
    ...
    icd10Code: padChars(
      firstFilled(
        form.icd10Code,
        padChars(truncate(upper(diag.primary_icd_code).replace(/\s/g, ""), 10), 10),
      ) ?? "",
      10,
    ),
    icd10PcsCode: padChars(
      firstFilled(
        form.icd10PcsCode,
        padChars(truncate(upper(firstFilled(diag.primary_icd_pcs_code, diag.procedure_1_icd_pcs)).replace(/\s/g, ""), 10), 10),
      ) ?? "",
      10,
    ),
    dateOfInjury: padChars(firstFilled(form.dateOfInjury, form.injuryDate) ?? "", 8),
    dateOfAdmission: padChars(firstFilled(form.dateOfAdmission, form.admissionDate) ?? "", 8),
    timeOfAdmission: padChars(firstFilled(form.timeOfAdmission, form.admissionTime) ?? "", 4),
    expectedDeliveryDate: dateOfDeliveryExpected,
    firNo: firNumber,
    billsRows,
    proposedTreatmentMedical: parseBool(firstFilled(form.proposedTreatmentMedical, diag.proposed_line_medical_management)) === true,
    proposedTreatmentSurgical: parseBool(firstFilled(form.proposedTreatmentSurgical, diag.proposed_line_surgical_management)) ?? procedureExists,
    proposedTreatmentIntensiveCare:
      parseBool(firstFilled(form.proposedTreatmentIntensiveCare, diag.proposed_line_intensive_care)) ??
      Number(firstFilled(hosp.icu_days, ext.admission_details?.icu_days) ?? 0) > 0,
    proposedTreatmentInvestigation: parseBool(firstFilled(form.proposedTreatmentInvestigation, diag.proposed_line_investigation)) === true,
    proposedTreatmentNonAllopathic:
      parseBool(firstFilled(form.proposedTreatmentNonAllopathic, diag.proposed_line_non_allopathic)) ??
      (systemOfMedicine ? !systemOfMedicine.includes("allopath") : false),
    drugAdministrationRoute: String(firstFilled(form.drugAdministrationRoute, diag.drug_administration_route) ?? "").trim().toLowerCase(),
    substanceTestDone: String(firstFilled(form.substanceTestDone, yesNo(diag.substance_test_conducted)) ?? "").trim(),
    isEmergencyHospitalization:
      parseBool(firstFilled(form.isEmergencyHospitalization, diag.is_emergency_hospitalization)) ??
      typeOfAdmission === "emergency",
    isPlannedHospitalization:
      parseBool(firstFilled(form.isPlannedHospitalization, diag.is_planned_hospitalization)) ??
...
function applyAutofillOverlayB(base, analysisData, data) {
  ...
  const patientName = splitName(
    firstFilled(partB.patient_name, patient.name),
  );
  ...
  out.hospitalId = padChars(
    truncate(String(firstFilled(partB.hospital_id, hosp.hospital_id, out.hospitalId) ?? ""), 20),
    20,
  );
  ...
  if (hospitalType) {
    out.hospitalNetwork = hospitalType.includes("non") ? "non_network" : "network";
  }
  ...
  out.qualification = String(
    firstFilled(partB.doctor_qualification, hosp.doctor_qualification, out.qualification) ?? "",
  ).trim();
  out.registrationNoStateCode = padChars(
    truncate(String(firstFilled(partB.doctor_registration_number, partB.registration_no, hosp.doctor_registration_number, out.registrationNoStateCode) ?? ""), 14),
    14,
  );
  out.phoneNo = padChars(
    truncate(
      digitsOnly(
        firstFilled(partB.treating_doctor_contact, partB.phone, hosp.doctor_phone, hosp.hospital_phone, out.phoneNo),
      ),
      11,
    ),
    11,
  ...
  out.ipRegNumber = padChars(
    truncate(String(firstFilled(partB.ip_registration_number, hosp.ip_registration_number, patient.patient_id, out.ipRegNumber) ?? ""), 15),
    15,
  );
  if (partB.gender) {
  ...
  if (partB.age_years != null) out.ageYears = padChars(parseAge(partB.age_years), 2);
  if (partB.age_months != null) out.ageMonths = padChars(truncate(digitsOnly(partB.age_months), 2), 2);
  if (partB.date_of_birth) out.dob = padChars(parseDateDDMMYY(partB.date_of_birth), 6);
  if (partB.admission_date) out.admissionDate = padChars(parseDateDDMMYYYY(partB.admission_date), 8);
  if (partB.admission_time) out.admissionTime = parseTime(partB.admission_time);
  if (partB.discharge_date) out.dischargeDate = padChars(parseDateDDMMYYYY(partB.discharge_date), 8);
  if (partB.discharge_time) out.dischargeTime = parseTime(partB.discharge_time);
  if (partB.type_of_admission) out.typeOfAdmission = mapAdmissionType(partB.type_of_admission);
  if (partB.date_of_delivery) out.dateOfDelivery = padChars(parseDateDDMMYY(partB.date_of_delivery), 6);
  if (partB.gravida_status != null) out.gravidaStatus = padChars(truncate(String(partB.gravida_status), 4), 4);
  if (partB.status_at_discharge) out.dischargeStatus = mapDischargeStatus(partB.status_at_discharge);
  out.totalClaimedAmount = String(
    firstFilled(partB.total_claimed_amount, claim.bill_amount, claim.claimed_amount, out.totalClaimedAmount) ?? "",
  ).trim();
  ...
  if (Array.isArray(partB.diagnoses)) {
    out.diagnoses = Array.from({ length: 4 }, (_, idx) => {
      ...
      return {
        icd10: padChars(
          truncate(upper(firstFilled(row.icd10, row.icd_code, row.code) ?? "").replace(/\s/g, ""), 7),
          7,
        ),
        description: String(firstFilled(row.description, row.label, row.name) ?? ""),
      };
    });
  }
  ...
  if (Array.isArray(partB.procedures)) {
    out.procedures = Array.from({ length: 4 }, (_, idx) => {
      ...
      return {
        icd10pcs: padChars(
          truncate(upper(firstFilled(row.icd10pcs, row.icd_pcs, row.code) ?? "").replace(/\s/g, ""), 7),
          7,
        ),
        description: String(firstFilled(row.description, row.label, row.name) ?? ""),
      };
  ...
  if (partB.pre_authorization_obtained != null) out.preAuthObtained = yesNo(partB.pre_authorization_obtained);
  if (partB.pre_authorization_number) out.preAuthNumber = padChars(truncate(String(partB.pre_authorization_number), 18), 18);
  if (partB.pre_authorization_missing_reason) out.preAuthMissingReason = String(partB.pre_authorization_missing_reason).trim();
  if (partB.hospitalization_due_to_injury != null) out.injuryHospitalization = yesNo(partB.hospitalization_due_to_injury);
  if (partB.injury_cause) {
    ...
    out.injurySubstance = cause.includes("alcohol") || cause.includes("substance");
  }
  if (partB.substance_test_conducted != null) out.substanceTestDone = yesNo(partB.substance_test_conducted);
  if (partB.is_medico_legal != null) out.medicoLegal = yesNo(partB.is_medico_legal);
  if (partB.reported_to_police != null) out.reportedPolice = yesNo(partB.reported_to_police);
  if (partB.fir_number) out.firNumber = padChars(truncate(String(partB.fir_number), 12), 12);
  if (partB.fir_not_reported_reason) out.firNotReportedReason = String(partB.fir_not_reported_reason).trim();

  if (Array.isArray(partB.claim_documents_checklist)) {
    out.claimDocChecklist = mapChecklistFlags(partB.claim_documents_checklist, 16);
  }

  out.nonNetAddress1 = padChars(
    truncate(upper(firstFilled(partB.hospital_address_line_1, hosp.hospital_address, out.nonNetAddress1), 45), 45),
    45,
  );
  out.nonNetAddress2 = padChars(
    truncate(upper(firstFilled(partB.hospital_address_line_2, out.nonNetAddress2), 50), 50),
    50,
  );
  out.nonNetCity = String(firstFilled(partB.hospital_city, hosp.hospital_city, out.nonNetCity) ?? "").trim();
  out.nonNetState = String(firstFilled(partB.hospital_state, hosp.hospital_state, out.nonNetState) ?? "").trim();
  out.nonNetPin = padChars(truncate(digitsOnly(firstFilled(partB.hospital_pin, hosp.hospital_pin, out.nonNetPin)), 6), 6);
  out.nonNetPhone = padChars(truncate(digitsOnly(firstFilled(partB.hospital_phone, hosp.hospital_phone, out.nonNetPhone)), 10), 10);
  out.nonNetRegStateCode = padChars(
    truncate(String(firstFilled(partB.hospital_registration_number, hosp.hospital_registration_number, out.nonNetRegStateCode) ?? ""), 14),
    14,
  );
  out.hospitalPan = padChars(
    truncate(upper(String(firstFilled(partB.hospital_pan, hosp.hospital_pan, out.hospitalPan) ?? "")).replace(/\s/g, ""), 10),
    10,
  );
  out.inpatientBeds = padChars(truncate(String(firstFilled(partB.inpatient_beds, hosp.inpatient_beds, out.inpatientBeds) ?? ""), 5), 5);
  if (partB.has_ot != null) out.facilityOT = yesNo(partB.has_ot);
  if (partB.has_icu != null) out.facilityICU = yesNo(partB.has_icu);
  if (partB.other_facilities) out.otherFacilities = String(partB.other_facilities).trim();
  if (partB.declaration_date) out.declarationDate = padChars(parseDateDDMMYYYY(partB.declaration_date), 8);
  if (partB.declaration_place) out.declarationPlace = upper(String(partB.declaration_place).trim(), 40);
