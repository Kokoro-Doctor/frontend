...
export function mapToFormA(analysisData) {
  ...
  const [addr1, addr2] = splitAddress(
    patient.address ?? patient.full_address,
    40,
    35,
  );
  const polCompact = truncate(
    upper(ins.policy_number ?? "").replace(/\s/g, ""),
    18,
  );
  ...
  const base = {
    ...
    diagnosis: String(
      diag.primary_diagnosis ?? diag.procedure_names?.[0] ?? "",
    ),
    bCurrentlyOther: "",
    ...
    hospPin: sh.pin,
    hospPhone: padChars(
      truncate(digitsOnly(patient.phone_secondary ?? patient.phone), 10),
      10,
    ),
    hospEmail: sh.email,
    ...
    injuryDate: padChars(
      parseDateDDMMYYYY(diag.date_of_injury_or_disease ?? hosp.date_of_injury),
      8,
    ),
    ...
    preHospPeriodDays: padChars(
      truncate(String(claim.pre_hosp_period_days ?? ""), 3),
      3,
    ),
    postHospPeriodDays: padChars(
      truncate(String(claim.post_hosp_period_days ?? ""), 3),
      3,
    ),
    domiciliary: "",
    ...
    accountNumber: padChars(
      truncate(String(bank.account_number ?? "").replace(/\s/g, ""), 22),
      22,
    ),
    bankNameBranch: padChars(
      upper(
        String(bank.bank_name ?? bank.bank_name_branch ?? bank.branch ?? ""),
        40,
      ),
      40,
    ),
    ...
    chequeDetails: String(
      bank.account_holder ?? bank.cheque_dd_payable_to ?? patient.name ?? "",
    ).trim(),
